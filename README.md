...
### Защита от cache stampede (SingleFlightCache)

`Vis\Builder\Services\SingleFlightCache` решает проблему **cache stampede** (он же dogpile): когда
горячий ключ сброшен или истёк по TTL, обычный `Cache::remember()` пускает в callback **все**
параллельные запросы одновременно — десятки/сотни одинаковых тяжёлых запросов разом бьют в БД.
Сервис гарантирует, что значение пересчитывает **только один** воркер, а остальные ждут готовый
результат (или получают «устаревшее» значение, пока идёт фоновое обновление).

Сервис регистрирует два макроса на кеш-репозитории (через `BuilderServiceProvider::register()`),
поэтому они доступны и через `Cache::`, и через `Cache::tags()`:

#### `rememberLocked()` — блокирующий single-flight (drop-in замена `remember`)

То же поведение и TTL, что у `remember()`/`rememberForever()`, плюс защита: на промахе строит один
воркер, остальные ждут результат. Без отдачи устаревших данных.

```php
// вместо Cache::remember($key, 3600, $cb)
Cache::rememberLocked('key', 3600, fn () => heavy());

// forever — передать null в качестве TTL:
Cache::rememberLocked('key', null, fn () => heavy());

// работает и с тегами:
Cache::tags(['products'])->rememberLocked('key', 1200, fn () => heavy());
```

#### `flexibleLocked()` — stale-while-revalidate + single-flight

Подходит для горячих страниц, где важна минимальная задержка. Поведение по состояниям значения:

- **свежее** (в пределах `$fresh`) — отдаём сразу;
- **устаревшее, но ещё в кеше** (между `$fresh` и `$total`) — отдаём старое значение **мгновенно**
  и запускаем обновление **в фоне** (после ответа пользователю), под блокировкой — обновляет один воркер;
- **холодный ключ** (значения нет) — строит один воркер под блокировкой, остальные ждут.

```php
// $ttl = [секунды_свежести, секунды_до_полного_протухания]
Cache::flexibleLocked('key', [3600, 3900], fn () => heavy());

Cache::tags(['products'])->flexibleLocked('menu', [86400, 90000], fn () => buildMenu());
```

> `flexibleLocked()` повторяет поведение нативного `Cache::flexible()` (Laravel 11.23+) и совместим
> с ним по сигнатуре (`$ttl = [fresh, total]`) — это drop-in. Дополнительно он берёт блокировку и на
> холодном ключе (нативный `flexible()` этого не делает), защищая от stampede даже после полного
> сброса кеша. Имя намеренно отличается от `flexible`, чтобы не конфликтовать с нативным методом и не
> ломаться при апгрейде до Laravel 12+.

**Где уже используется в пакете:** `setting()` (хелпер настроек) и query-кеш билдера
(`Builder::getCached()`) — два самых нагруженных пути.

**Производительность:** на свежем значении блокировка **не берётся вообще** (быстрый путь — обычный
`get`), поэтому горячий трафик не замедляется. Сервис рантайм-адаптивен: фоновое обновление идёт через
`defer()` под Octane (L11.23+) либо через `app()->terminating()` под PHP-FPM.
